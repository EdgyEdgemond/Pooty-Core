...
class adLDAP {
    ...
    * @var string
    */   
	protected $_account_suffix = "@salmat.com.au";
    ...
    * @var string
    */
	protected $_base_dn = "DC=salmat,DC=com,DC=au";
    ...
    * @var array
    */
    protected $_domain_controllers = array ("mel0ads01lv.salmat.com.au");
    ...
    * @var string
    */
	protected $_ad_username="ccs_service_ldap";
  protected $_ad_password="R34dMyLd4p!";
    ...
    * @param array $filters What to search on.
    * @param bool $sorted Sort the user accounts
    * @return array
    */
    public function all_users($include_desc = false, $search = array("*"), $filters = array("cn"), $fields=array("samaccountname","displayname"), $sorted = true){
        if (!$this->_bind){ return (false); }
        ...
        $filter = "(&(objectClass=user)(samaccounttype=". ADLDAP_NORMAL_ACCOUNT .")(objectCategory=person)$filterText)";
        ...
        for ($i=0; $i<$entries["count"]; $i++){
            if ($include_desc && strlen($entries[$i]["displayname"][0])>0){
              foreach($fields as $f) {
                $users_array[ $this->username2guid($entries[$i]["samaccountname"][0])][$f] = $entries[$i][$f][0];
              }
            } else {
